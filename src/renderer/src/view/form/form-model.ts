import { Nullish } from "@renderer/utils";
...
export const validator: FormValidator<WorkFormValues> = (values: WorkFormValues, t: TFunction) => {
	...
	if (!values.taskPrecentage || (0 > values.taskPrecentage && 100 < values.taskPrecentage)) {
		errors["taskPrecentage"] = t("common.error.fieldOutOfBounds");
	}

	if (!values.progress || values.progress?.trim().length <= 0) {
